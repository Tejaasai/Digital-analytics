...
select '31-12-2014' as period_upto, sum(revenue)-sum(cogs_usd) as total_profit
 ...
 --profit percentage
 SELECT (sum(Revenue) - SUM(cogs_usd))*100.00/sum(revenue) AS profit_percent
FROM table_1 
 where order_item_refund_id is null
...
SELECT 
    primary_product_id,product_name, 
    COUNT(order_item_id) AS primary_product_sales
FROM 
    table_1
WHERE 
    is_primary_item = 1
GROUP BY 
    primary_product_id, product_name
order by primary_product_sales desc

-- First purchase of customer from creation of product
SELECT 
    user_id,
    DATEDIFF(DAY, MIN(product_creation_time), MIN(order_creation_time)) AS days_to_first_purchase
FROM 
    table_1
GROUP BY 
    user_id;
...
SELECT 
    product_id,
    SUM(price_usd - cogs_usd) AS gross_profit_per_product
FROM 
    table_1
GROUP BY 
    product_id;
...
--** Questions from Investor's deck **--


--First 6 questions(based on Monthly,quarterly trends and seasonality)
...
select *,(curr_total_session-prev_total_session)*100/curr_total_session as
...
--non-brand search overall(quarter_wise)
...
select concat(year(b.created_at),'-',right('0'+ cast(month(b.created_at) as varchar(2)),2))Month_Year,count(distinct a.order_id) count_of_orders
...
count(DISTINCT a.order_id) count_of_orders,count(distinct b.website_session_id) count_of_website_session,b.device_type
from website_sessions as b
left join table_1 a
on a.user_id=b.user_id
and a.website_session_id=b.website_session_id
...
select  concat(year(session_creation_time),'-',right('0'+ cast(month(session_creation_time) as varchar(2)),2))Month_Year,
count(distinct website_session_id) as count_of_sessions,
utm_source as traffic_source
from table_2
group by month(session_creation_time),year(session_creation_time),utm_source
...
/*1.Understanding which marketing channels are driving the most sessions and orders through your website.*/
---page11---
...
--select SUM(CASE WHEN B.PAGEVIEW_URL='/thank-you-for-your-order' THEN A.REVENUE END) from table_1 a
--RIGHT join table_2 b
--on a.user_id=b.user_id
--and a.website_session_id=b.website_session_id

--select count(distinct order_id) from table_1
--where a.order_item_refund_id is null

--select count(distinct order_item_refund_id) from table_1

--select distinct(order_id) from order_item_refunds
...
/*6.Identify the most and least viewed pages by the customer to make creative decisions on the enhancement of the pages.*/
---page26---
...
WITH user_paths AS (
    SELECT 
        ...
        b.pageview_url,
        ROW_NUMBER() OVER (PARTITION BY b.user_id, b.website_session_id ORDER BY b.webpage_creation_time ASC) AS rn,
        MIN(b.webpage_creation_time) OVER (PARTITION BY b.user_id, b.website_session_id) AS session_start,
        MAX(b.webpage_creation_time) OVER (PARTITION BY b.user_id, b.website_session_id) AS session_end
    FROM table_2 b
...
bounce_data AS (
    ...
    FROM user_paths
    GROUP BY user_id, website_session_id,session_start, session_end
)

SELECT 
    COUNT(CASE WHEN page_count = 1 AND session_duration < 10 THEN 1 END) AS number_of_bounces,
    COUNT(*) AS total_sessions,
    (COUNT(CASE WHEN page_count = 1 AND session_duration < 10 then 1 END) * 100.0 / COUNT(*)) AS bounce_rate
FROM bounce_data;


/* customers visited only one page ands did not further proceed which is less than 10 seconds*/
...
where http_referer='null'
union
select 'referral_search' as visit_source,count(distinct rv.[user_id]) as total_user					  
from repeat_visitors as rv
join website_sessions as w
on rv.[user_id]=w.[user_id]
where http_referer!='null'
...
-----Customer cohort (those who made final order)
...
---User cohort (for all website visitors)
...
SELECT CAST((COUNT(REPEATED_CUSTOMERS)*100.00)/(SELECT COUNT(DISTINCT [user_ID])FROM table_1) AS FLOAT)
FROM(
SELECT [user_id] AS REPEATED_CUSTOMERS,COUNT(DISTINCT ORDER_ID) AS DISTINCT_ORDER_COUNT
FROM table_1
GROUP BY [user_id]
HAVING COUNT(DISTINCT ORDER_ID)>1 and SUM(revenue)>0
)
AS X
...
with cte1 as(SELECT 
    ...
    GROUP BY 
        [user_id]
    HAVING 
        COUNT(DISTINCT order_id) > 1 and sum(revenue)>0),
cte2 as(
...
--first time visit vs Repeat visit conversion rate
...
--Customer Lifetime Value--(slide no. 123)
...
/*1.	Analyzing Channel Portfolios: What are the weekly sessions data for both gsearch and bsearch from August 22nd to November 29th?*/
---page31-----
...
/*4.	Channel Portfolio Trends: Retrieve the data for gsearch and bsearch non-brand sessions segmented by device type 
...
/*8.	Cross-Sell Analysis: Analyze the impact of offering customers the option to add a second product on the /cart page, 
comparing the metrics from the month before the change to the month after?Specifically, in comparing the click-through rate (CTR) 
from the /cart page, average products per order, average order value (AOV), and overall revenue per /cart page view.  */


/*9. 	Portfolio Expansion Analysis: Conduct a pre-post analysis comparing the month before and the month after the launch of the “Birthday Bear” 
